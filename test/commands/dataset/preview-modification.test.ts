...
describe('dataset preview-modification', () => {
  beforeEach(() => {
    setupTestConfig()
    mockApi([{method: 'POST', path: '/v2/datasets/123/modifications/preview', response: {data: {items: [], pagination: {page: 0, pageSize: 25, totalItems: 0}}, requestId: 'test', status: 'success'}}])
  })
  afterEach(() => {
    cleanupTestConfig()
    restoreApi()
